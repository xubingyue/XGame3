...
struct POINT    // 如果不是windows环境则定义一下POINT结构
{
	...
	static POINT Normalize(const POINT &p1, const POINT &p2)
	{
		int tx = p2.x - p1.x;
		int ty = p2.y - p1.y;
...
<
		//class    _Map,                                                  // 地图 
		typename _BlockTestFunc,    // 阻挡判断函数对象,函数型如bool IsBlock(POINT pt)
...
public:
	...
	@return         :
	*/
	bool SetMapInfo(unsigned long nMapWidth, unsigned long nMapHeight, _BlockTestFunc  *BlockFunc);
...
public:
	...
	bool IsBevelBlock(const POINT & ptTile, const POINT & ptNext);
...
#define  TEMPLATE_DECLARE		template<typename _BlockTestFunc,int MAX_PATH_LEN,int MAX_SEARCH_AREA>
#define  PATH_FINDER_DECLARE	PathFinder<_BlockTestFunc,MAX_PATH_LEN,MAX_SEARCH_AREA>
...
//////////////////////////////////////////////////////////////////////////
//TEMPLATE_DECLARE
//bool PATH_FINDER_DECLARE::SetMapInfo(unsigned long nMapWidth, unsigned long nMapHeight, _Map * pMap, _BlockTestFunc *blockFunc)
TEMPLATE_DECLARE
bool PATH_FINDER_DECLARE::SetMapInfo(unsigned long nMapWidth, unsigned long nMapHeight, _BlockTestFunc *blockFunc)
{
...
bool PATH_FINDER_DECLARE::FindPath(const POINT & ptStart, const POINT & ptEnd, POINT *& pPath, int & nPathLen, int nOption, int nMaxStep)
{
	...
	int  nShortest = -1;
	bool bFindMore = true;
	int tmp1 = (int)(ptEnd.x - ptStart.x);
	int tmp2 = (int)(ptEnd.y - ptStart.y);
	int  dx = ABS(tmp1);
	int  dy = ABS(tmp2);
	int tmp3 = dx - dy;
	int  nPathLenLimit = std::min(dx, dy) + ABS(tmp3) + 5;
	...
	{
		if ((m_nStepCount > nPathLenLimit) && (nOption & SEARCH_OPTION_OPTIMIZE))
		{
			ConnectCorners(ptStart);
		}
...
//
TEMPLATE_DECLARE
bool PATH_FINDER_DECLARE::IsBevelBlock(const POINT & ptTile, const POINT & ptNext)
{
	if ((ptTile.x - ptNext.x == 0) || (0 == ptTile.y - ptNext.y))
	{
		// 不是斜边
		return false;
	}

	return m_BlockFunc->IsBlock(POINT(ptNext.x, ptTile.y) * WAY_UNIT) || m_BlockFunc->IsBlock(POINT(ptTile.x, ptNext.y) * WAY_UNIT);
}
...
bool PATH_FINDER_DECLARE::Approach(const POINT & ptStart, const POINT & ptEnd)
{
	...
	while (++m_nStepCount <= m_nMaxStep)
	{
		...
		//////////////////////////////////////////////////////////////////////////
		// 增加斜边判断 - ZhongShan - 2015/08/05 -

		bool isBevel = IsBevelBlock(ptTile, ptNext);
		// 碰到阻挡开始绕转
		if (m_BlockFunc->IsBlock(ptNext * WAY_UNIT) || isBevel)
		{
			// 如果目标点是阻挡的则直接退出
...
bool PATH_FINDER_DECLARE::Around(const POINT & ptLoc, int nMoveDir, int nTurnDir, const POINT & ptStart, const POINT & ptEnd)
{
	...
	while (++m_nStepCount <= m_nMaxStep)
	{
		...
		bool bNextBlock = m_BlockFunc->IsBlock(ptNext * WAY_UNIT);
		bool isBevel = IsBevelBlock(ptTile, ptNext);
		if (!bNextBlock)
		{
			bNextBlock = isBevel;
		...
		isBevel = false;
		isBevel = IsBevelBlock(ptAround, ptTile);
		if (!m_BlockFunc->IsBlock(ptAround * WAY_UNIT) && !isBevel)
		{
			nMoveDir = TurnDir(nMoveDir, nTurnDir);
		}
	};
...
bool PATH_FINDER_DECLARE::ConnectDirect(const POINT & ptStart, const POINT & ptEnd)
{
	...
	while (++m_nStepCount <= m_nMaxStep)
	{
		...
		bool bNextBlock = IsBevelBlock(ptTile, ptNext);

		// 碰到阻挡放弃
		if (m_BlockFunc->IsBlock(ptNext * WAY_UNIT) || bNextBlock)
		{
			return false;
		}
...
void PATH_FINDER_DECLARE::ConnectCorners(const POINT & ptStart)
{
	...
	while (1)
	{
		for (int i = 0; i< (int)CORNER_NUM; ++i)
		{
...
bool PATH_FINDER_DECLARE::FindAStar(const POINT & ptStart, const POINT & ptEnd)
{
	...
	while (++m_nStepCount <= m_nMaxStep)
	{
		...
		for (int dir = 0; dir<DIR_MAX; ++dir)
		{
			...
			int tmp1 = (int)(ptNext.x - ptEnd.x);
			int tmp2 = (int)(ptNext.y - ptEnd.y);
			nDistArray[dir] = ABS(tmp1) + ABS(tmp2);
			if ((unsigned long)nDistArray[dir]<nNearestDist)
			{
				m_pNearestTile = pTag;
...
bool PATH_FINDER_DECLARE::IsOnStraightWay(const POINT & ptTile, const POINT & ptStart, const POINT & ptEnd)
{
	...
	if (abs_x == abs_y)
	{
		if (ABS((int)(ptTile.x - ptStart.x)) != ABS((int)(ptTile.y - ptStart.y)))
			return false;
	...
	while (m_pNearestTile && nPathLen<nBufSize)
	{
		...
		if (optmize)
		{
			if (m_pNearestTile->bAround && nPathLen >= 2)
			{
				...
				if (ABS((int)(ptPreTile.x - ptTile.x)) <= 1 && ABS((int)(ptPreTile.y - ptTile.y)) <= 1)
				{
					nPathLen -= 1;
				}
			}
